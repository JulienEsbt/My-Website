...
export default function HomeTravelCarousel({language, readLabel}) {
    ...
    const [index, setIndex] = useState(() => Math.max(0, destinations.findIndex(trip => trip.id === state?.homeTrip)))
    ...
    const select = value => {setPaused(true); setIndex((value + destinations.length) % destinations.length)}
    return (
        <div ref={root} className="home-travel-carousel" role="region" aria-label={fr ? 'Récits de voyage sélectionnés' : 'Selected travel stories'}
            onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <article className="home-discover__travel" key={trip.id} onFocusCapture={() => setPaused(true)}>
                <div className="home-discover__photo">
                    <ResponsiveImage media={trip.photo} alt={`${fr ? trip.country : trip.countryEn} · ${trip.year}`} sizes="(max-width: 700px) 90vw, 55vw" />
                    <span className="home-discover__location"><FiMapPin aria-hidden="true" />{fr ? trip.country : trip.countryEn}</span>
                </div>
                <div className="home-discover__travel-copy">
                    ...
                    <Link className="btn home-discover__story-link" to={`/travel/${trip.id}`} state={{fromHome: 'travel'}}>
            ...
            <div className="home-travel-carousel__controls">
                <div className="home-travel-carousel__destinations">
                    {destinations.map((entry, i) => <button key={entry.id} type="button" aria-pressed={i === index} onClick={() => select(i)}>
                        <span aria-hidden="true">0{i + 1}</span> {fr ? entry.country : entry.countryEn}
                    </button>)}
                </div>
                <div className="home-travel-carousel__arrows">
                    <button type="button" onClick={() => select(index - 1)} aria-label={fr ? 'Voyage précédent' : 'Previous trip'}><FiArrowLeft /></button>
                    {!reducedMotion && <button type="button" onClick={() => setPaused(value => !value)} aria-label={paused ? (fr ? 'Reprendre le défilement des voyages' : 'Resume travel slideshow') : (fr ? 'Mettre les voyages en pause' : 'Pause travel slideshow')}>{paused ? <FiPlay /> : <FiPause />}</button>}
                    <button type="button" onClick={() => select(index + 1)} aria-label={fr ? 'Voyage suivant' : 'Next trip'}><FiArrowRight /></button>
                </div>
            </div>
        </div>
    )
