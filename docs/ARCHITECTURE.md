...
Citizen resources use versioned metadata in `src/data/citizenResources/resources.js` and the lazy-loaded `resources` translation namespace. `/resources` and `/en/resources` share the existing route registry, prerender, sitemap and navigation. The section adds no API, account, remote embed or analytics integration. Static, credited previews live in `public/previews/citizen-resources`; the media ownership map remains a future project, not a dependency.
